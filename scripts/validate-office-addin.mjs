...
for (const file of manifestFiles) {
  ...
  const checks = [
    ...
    [manifest.includes('<bt:Set Name="AddinCommands" MinVersion="1.1"/>'), 'manifest must require AddinCommands 1.1 for ribbon buttons'],
    ...
    [manifest.includes('<OfficeTab id="TabHome">'), 'manifest must add the SlideEngage command to PowerPoint Home tab'],
    [manifest.includes('<Group id="SlideEngageCommandsGroup">'), 'manifest must include a SlideEngage command group'],
    [manifest.includes('<Control xsi:type="Button" id="SlideEngageOpenTaskpaneButton">'), 'manifest must include an Open SlideEngage button'],
