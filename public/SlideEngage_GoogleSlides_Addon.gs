...
function buildSidebarHtml_() {
  return '<!doctype html><html><head><base target="_top"><style>' +
    ...
    'function renderInteractions(){var list=$("interactions");list.innerHTML="";state.interactions.forEach(function(i){var row=document.createElement("div");row.className="item"+(state.selectedInteraction&&state.selectedInteraction.id===i.id?" active":"");row.innerHTML="<b>"+(i.title||"Untitled")+"</b><div class=\\"small muted\\">"+label(i)+" · "+(i.status||"draft")+"</div><div class=\\"row\\" style=\\"margin-top:8px\\"><button class=\\"btn secondary\\" data-action=\\"edit\\">Edit</button><button class=\\"btn danger\\" data-action=\\"delete\\">🗑 Delete</button></div>";row.onclick=function(){openInteraction(i)};row.querySelector("[data-action=edit]").onclick=function(e){e.stopPropagation();openInteraction(i)};row.querySelector("[data-action=delete]").onclick=function(e){e.stopPropagation();deleteInteractionFromList(i)};list.appendChild(row)})}' +
