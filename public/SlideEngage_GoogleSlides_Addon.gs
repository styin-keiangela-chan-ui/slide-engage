...
function buildSidebarHtml_() {
  return '<!doctype html><html><head><base target="_top"><style>' +
    ...
    '<section id="login" class="card stack"><b>Lecturer login</b><div class="small muted">Use the same SlideEngage website email/Gmail and password.</div><div id="permission" class="status permission hidden"></div><button class="btn secondary" onclick="authorize()">Authorize connection</button><input id="email" class="input" placeholder="Email"><input id="password" class="input" type="password" placeholder="Password"><button class="btn" onclick="login()">Sign in</button></section>' +
    ...
    'function authorize(){call("authorizeSlideEngage",[],function(){checkAuthorization();show("SlideEngage is authorized to connect to the internet.")})}function login(){call("login",[$("email").value,$("password").value],function(r){state.session=r;refresh()})}function logout(){call("logout",[],function(){state={session:null,events:[],selectedEvent:null,interactions:[],selectedInteraction:null,template:null,options:[]};render()})}' +
